...
for (i in 1:length(explist)) {
  ...
  exposure = exposure[-which(is.na(exposure[,2])),]  # 去除缺失值
  d = merge(merge(prs,exposure,by='n_eid'),outcome,by='n_eid') # 同时具有IV,exposure,outcome的数据
  
  d = d[which(d$n_eid %in% qc_pass),]  # QC通过
  
  d = d[which(d$n_eid %in% cov_batch$n_eid),]
  
  data = merge(d,cov_batch,by='n_eid') # 合并协变量数据
...
exposure = exposure[-which(is.na(exposure$n_4080_0_0) & is.na(exposure$n_4080_0_1)),] # 去除两次测量都没有的
