...
var_cal_gaussian = function(object1,object2,y,i,data1) 
  # object1: stage 1 model
  # object2: gam model
  {
  Xp = predict(object2,type='lpmatrix')
  c2 = lm(y ~ Xp-1)
  rho = c2$coefficients[[which(names(c2$coefficients)=='Xpr')]]
  ...
  d = var(ehat) + rho^2 * (as.numeric(predict(object1,data1,se=TRUE)$"se.fit"))^2
  S = (object2$smooth[[i]])$S[[i]]
  ...
  lambda = object2$sp[[i]]
  W1 = sqrt(d) * W
  V = solve(crossprod(W)+lambda*S) %*% (t(W1)%*%W1) %*% solve(crossprod(W))
...
simu = function(PVE,N) {
  ...
  for (i in 1:500) {
    ...
    c1 = lm(x ~ z + c)
    r = c1$residuals
    o = gam(y ~ s(x,bs="cr") + c +r,family = "gaussian")
    
    V = var_cal_gaussian(c1,o,y,1)
    p[i] = p_cal(o,V,1)
    ...
    V = model.matrix(c1)
    V_beta = vcov(c1,freq = TRUE)
    D = diag(rep(as.numeric(var(r2)),N)) + rho^2 * V %*% V_beta %*% t(V)
    se = sqrt((solve(t(W)%*%W) %*% (t(W) %*% D %*% W) %*% solve(t(W)%*%W))[2,2])
...
qqplot(p_u,r_25_n1_x2[,1],type='l',lty=1,ylim = c(0,1),
       xlab = " ",ylab=" ",cex.lab = 1.3,cex.axis=1.3)
par(new=T)
qqplot(p_u,r_25_n1_x2[,2],type='l',lty=2,ylim = c(0,1),
       xlab = "Uniform quantiles",ylab="Ordered p-values",cex.lab = 1.3,cex.axis=1.3)
...
write.table(r_25,'./smoothingP0_0.25.txt',
...
write.table(r_75,
            './smoothingP0_0.75.txt',
            sep=' ',quote = FALSE,row.names = FALSE)

library(data.table)
r_25 = fread('C:/ShareCache/王鑫培_2111110203/AAA小论文/2 - nonlinearMR/Simulation/smoothingP0_0.25.txt')
r_50 = fread('C:/ShareCache/王鑫培_2111110203/AAA小论文/2 - nonlinearMR/Simulation/smoothingP0_0.50.txt')
r_75 = fread('C:/ShareCache/王鑫培_2111110203/AAA小论文/2 - nonlinearMR/Simulation/smoothingP0_0.75.txt')
